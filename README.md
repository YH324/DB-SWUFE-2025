...
- [基础sql](#w3-sql)
- [基础sql操作](#w4-上机)
- [聚合查询与增删改](#w5-聚合查询与增删改)
- [JOIN与查询优化](#w7-多表JOIN与查询优化)
- [函数与过程、数据库设计](#w9-函数与过程数据库设计)
- [Python连接数据库](#w11-Python连接数据库与操作)
- [ER与NF](#w12-ER模型函数依赖与范式分解)
- [theory](#w14-存储索引与事务)
- [期末设计](#w14_16-期末设计)
...
我负责的任务是**检查SQL查询语句的正确性，如果查询语句有错误，能够给出错误的原因，要求报错信息尽可能用户友好，并且能够修改建议**，一开始天然觉得这个任务模块很适合LLM agent实现，所以在一些低代码平台进行工作流设计和调试。最初的想法是python工具包判断语法错误+LLM优化输出，但在实际操作过程中遇到了不兼容等问题暂时作罢。
...
作为数据科学与大数据技术专业学生，在《数据库原理与应用》的学习过程中，我感到充实而有挑战。课程中涵盖的内容，从基础的关系建模到复杂的范式分解、再到实际数据库存储与事务处理，体系严谨，实践导向强。通过本学期的学习和项目实践，我深刻体会到数据库系统不仅仅是简单的表格管理，它背后蕴含的是严密的逻辑理论与高效的工程实现。对于未来可能持续扎根在数据科学领域的我而言，在这门课程中收获的许多数据管理的知识和方法都十分有益。

在这门课的学习中，我付出了持续的努力，对于每项作业和上机任务都认真对待，课后也进行了知识点的梳理。然而我深感自己在这门课程上的付出还有不足，课外的探索不足，也比较依赖llm，有时候比起去翻阅网站资料，更喜欢通过llm检索找答案。这并不是一个很好的习惯。此外，我在课堂上的效率也并不高，对此我深感遗憾。

我赞同老师说的“我们要警惕成为知道分子，即知道很多似是而非的概念，但没有自己深入见解”和“避免被工具化但也不能局限于使用工具”，我未来也会以此为戒，人说llms dont know what they dont know，我未来会更加去探索what i dont know，避免陷入知道分子的困境

最终项目自评成绩为 **47/50**，我认为这是对我真实投入的肯定。

**扣除的3分主要原因如下：**
1. 上课思考不足，依赖llm，理解不到位；
2. 上机测试盒中期测试完成度不高；
3. 课外探索程度低，懒。

我接受这3分的扣除，因为它提醒我不仅要学得扎实，还要表达得准确严谨。与此同时，我也为我为这门课付出的努力感到骄傲。

最后，非常感谢老师这一学期的辛勤教导与付出，虽然课程即将结束，但老师严谨的教学态度以及对于ai时代下知识学习的思考深深影响了我，我相信我在这门课程的收获已经超出学科范畴本身，再次感谢老师。祝好。
...
在肯定收获的同时，我也必须坦诚地反思自身的不足。我为这门课程付出了持续的努力，认真完成了每一项作业与上机任务，并在课后进行了系统的知识梳理。然而，我的投入程度距离“优秀”仍有差距。主要体现在三个方面：
   - 首先，课堂学习效率有待提高，有时未能跟上老师的思路进行深度思考；
   - 其次，课外的探索与拓展不足，主动性欠缺，存在一定的学习惰性；
   - 最后，在学习方法上，我有时会过度依赖LLM进行快速检索，而忽视了查阅一手资料与官方文档的重要性。这种寻求“捷径”的习惯，虽然在短期内解决了问题，却阻碍了知识的深度内化，这是一个我必须警惕并加以改正的倾向。

我对老师提及的`“我们要警惕成为‘知道分子’，即知道很多似是而非的概念，但没有自己深入的见解”`、`“避免被工具化，但也不能局限于使用工具”`的观点深有感触，对LLM的不当依赖，恰是陷入这种困境的捷径。有人曾说 _LLMs don't know what they don't know_ ，真正的学习者需要主动去探索 _what I don't know_ 的未知领域。未来，我将以此为戒，培养更为扎实和独立的学习习惯，努力避免成为浮于表面的`“知道分子”`。

基于以上的学习表现与自我反思，我为自己的最终项目自评为47分。我为自己在这门课程中付出的努力和取得的进步感到自豪，这份成绩是对我投入的肯定。同时，我也坦然接受扣除的分数，它清晰地指向了我的不足之处：
   - 课堂思考与知识内化不足：对LLM存在依赖，部分概念理解不够透彻；
   - 实践环节完成度欠佳：上机测试与中期测试的准备与表现有待提升；
   - 学习主动性与探索精神不足：课外自主学习的深度和广度不够。

最后，感谢老师这一学期的辛勤教导与付出。虽然教学即将结束，但老师严谨的教学态度，以及对AI时代下知识学习的洞见已经影响了我并促使我反思，我相信我在这门过程中的收获已经超出课程本身，并将使我长久收益。再次感谢老师，祝未来一切顺利。
